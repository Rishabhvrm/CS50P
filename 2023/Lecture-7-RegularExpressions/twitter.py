...
if matches := re.search(r"^https?://(?:www\.)?twitter\.com/(.+)$", url, re.IGNORECASE):     # here we specify that we don't need www. therefore it will not return its value and we can get the value in matches.group(1) instead of matches.group(2)
